...
class data_generator:
    ...
    def pro_res(self):
        idxs = list(range(len(self.data)))  #一共多少个训练数据
        # print(idxs)
        np.random.shuffle(idxs)  # 全部打乱
        T, S1, S2, K1, K2, O1, O2, = [], [], [], [], [], [], []  # 定义数组
        for i in idxs:
            d = self.data[i]  # 拿到每一个
            text = d['text']
            items = {}
            for sp in d['spo_list']:
                subjectid = text.find(sp[0])  # 拿到头尾实体在句子中的位置
                objectid = text.find(sp[2])
                if subjectid != -1 and objectid != -1:  # 头尾都有
